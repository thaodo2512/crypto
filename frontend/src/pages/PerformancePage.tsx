...
function AccuracyBar({ label, value }: { label: string; value: number }) {
  ...
  const color =
    pct >= 60 ? "#00d4aa" : pct >= 45 ? "#f59e0b" : "#ff4757";
  ...
  if (perfLoading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-text-muted animate-pulse">Loading performance...</div>
      </div>
    );
  }
      ...
      {perf?.win_rate && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <MetricCard
            ...
            color={perf.win_rate.win_rate >= 0.55 ? "#00d4aa" : perf.win_rate.win_rate >= 0.45 ? "#f59e0b" : "#ff4757"}
          />
          <MetricCard
            label="Wins"
      ...
      {perf?.win_rate && perf.win_rate.total > 0 && (
        <div className="bg-bg-card rounded-lg border border-border-subtle p-5">
          ...
          <div className="h-6 rounded-full overflow-hidden flex">
            <div
              className="h-full transition-all duration-700 flex items-center justify-center text-[10px] font-bold"
              style={{
                ...
                {Object.entries(perf.regime_accuracy)
                  ...
                  .map(([regime, accuracy]) => {
                    ...
                    const color =
                      pct >= 60 ? "#00d4aa" : pct >= 45 ? "#f59e0b" : "#ff4757";
                    return (
                      <tr key={regime} className="border-b border-border-subtle/50">
                        <td className="py-2.5 px-2 font-medium capitalize">
                          {regime.replace(/_/g, " ")}
                        </td>
                        <td className="text-right py-2.5 px-2" style={{ color }}>
                          {pct.toFixed(1)}%
                        </td>
        ...
        <div className="bg-bg-card rounded-lg border border-border-subtle p-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
            <MetricCard
              ...
              color={health.status === "ok" ? "#00d4aa" : "#ff4757"}
            />
            <MetricCard
              label="Uptime"
          ...
          {health.collectors && Object.keys(health.collectors).length > 0 && (
            <div>
              <h3 className="text-text-muted text-[10px] uppercase tracking-wider mb-2">
                Data Collectors
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-1">
                {Object.entries(health.collectors).map(([name, info]) => (
                  <div
                    key={name}
                    className="flex items-center justify-between py-1.5 px-2 rounded bg-bg-primary text-xs"
                  >
                    ...
                    <span
                      ...
                      style={{
                        color: info.status === "ok" ? "#00d4aa" : "#ff4757",
                      }}
                    >
